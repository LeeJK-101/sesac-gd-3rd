...
const dinnerMenu = ['라면', '볶음밥', '바질페스토 파스타'];
...
//for
for (let index = 0; index < lunchMenu.length; index++) {
  console.log(lunchMenu[index]);
}

//forEach  콜백함수(인자)=>{}
lunchMenu.forEach((menu) => {
  console.log('forEach:', menu);
});
...
// 전체 연산자 사용 -> 일일이 요소에 접근하지 않고 사용할 수 있도록 함.
console.log(...lunchMenu);
console.log(...[1, 2, 3, 4, 5]); // 꼭 변수에 저장하지 않고 배열에 바로 사용 가능

//배열 합치기
//['국밥', '치킨마요', '파스타', '돈까스','라면', '볶음밥', '바질페스토 파스타']

const newArr = [];

function usePush() {
  lunchMenu.map((menu) => newArr.push(menu)); // 화살표 함수 한줄로 끝나면 중괄호 없이 할 수 있음.
...
// 객체를 합칠 때 동일한 key가 있다면 뒤에 이는 변수 객체의 key의 value가 저장됨
let me = { ...me1, ...me2 }; //객체를 유지한채로 객체로 합쳐추려고 {}를 사용
console.log(me);
...
// 다른 방법 2
// 문자를 concat 메서드 사용해 배열 연결하고 배열로 저장
...
// console.log(yellow); // 에러

// 가장 마지막 요소를 변수에 저장하지 않을 경우 쉼표 없이 생략하면 됨
const [c1, , c3] = colors;
console.log(c1); //red
console.log(c3); //yellow
...
const [
  ccc1,
  ccc2,
  ccc3 = 'cc3 default', //defult 값이 사용되지 않고,colors 배열의 2번 인덱스인 'yellow'가 저장됨
  ccc4,
...
value1 = value2; //->value1 변수의 값은 'secound'
vaule2 = value1; //->value2 변수의 값은 'first'
...
function addNumber(...num) {
  console.log(num); // 전달받은 모든 인자는 num이라는 배열에 저장되어 있음
...
function introduce(name, like = 'money', hate, gender = '여') {
  console.log(
    `안녕하세요~ 제 이름은 ${name} 이고, 저는 ${like}를 좋아합니다. 하지만 ${hate}를 싫어해요. 아 참! 그리고 저는 ${gender}성입니다.`
  );
}
...
introduce('레일라'.undefined, '운동'); // undefined를 넣는 식으로 이렇게 사용하는 경우 거의 없음.
