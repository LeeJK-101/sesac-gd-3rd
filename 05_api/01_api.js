...
// 만약 비동기 코드를 동기적으로 또는 원하는 순서대로 실행시키ㅣ고 싶다면?
...
// async & await 사용
// promise는 then 을 체이닝 할 수 있어 너무 많은 체이닝을 할 경우 가독성이 떨어짐
...
// //Promise를 함수에 담아 해당 함수 호출시 Promise가 실행되도록 작성
// const promiseFunc = () => {
...
// promise .then은 성공했을 때 실행됨. .then보다 가독성 좋음. async try{}를 쓰는 이유 */*/*/*

//---------------------------------------------------------------------------------

// json
// 데이터를 전송하기 위한 경량의 데이터 포맷 (굉장히 가벼움)
// 여러 언어에서 쉽게 사용할 수 있어 서버와의 통신에서는 json으로 통신합니다.
...
async function getMovieData() {
  ...
  const response = await fetch(
    `http://www.omdbapi.com/?i=tt3896198&apikey=#API에이피아이키#&s=${word}`
  );
  const movieData = await response.json();
  console.log(movieData.Search);

  const ul = document.querySelector('ul');
  // 불러온 데이터는 movieData.Search 배열 안에 각가그이 객체로 있음
  ...
  for (let movie of movieData.Search) {
    ...
    const p = document.createElement('p'); // 영화 ID
...
const postPost = () => {
  fetch('https://jsonplaceholder.typicode.com/posts/', {
    ...
    body: JSON.stringify({
      title: '제목제목',
      body: '게시글 내용 내용 내용 내용',
      userId: 1,
    }),
    headers: {
      'content-type': 'application/json; charset=UTF-8',
      // 자바스크립트 객체. JSON으로 바꾸야함
    },
  })
    .then((response) => response.json())
    .then((data) =>
...
const deletePost = () => {
  fetch('http://jsonplaceholder.typicode.com/posts/1', {
    method: 'DELETE', // 두번째인자는 길이가 길어질 수 있어서 {}로 묶음.
  })
    .then((response) => response.json())
