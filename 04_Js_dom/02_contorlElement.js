/*
1. 태그 내부 content 다루기
- innerText : 요소 안의 텍스트를 가져오거나 수정(다른 문자열로 변경)
...
div1.innerHTML =
  '여기는 첫 번째 div 태그이면서 js에서 <strong>innerHTML을 사용해 수정</strong>했습니다..';
...
// 클래스 가져오기
console.log(flowers[0].classList); //DOMTokenList(2) ['pink', 'changeStyle', value: 'pink changeStyle']

// 클래스 지우기
flowers[0].classList.remove('changeStyle'); // 첫번째 요소에서 스타일 요소를 제거

// 클래스 가지고 있는지 여부 확인 -> t/f -> contains는 조건문에서 사용
console.log(flowers[0].classList.contains('changeStyle')); // true 첫번째 요소에서 특정 스타일 있는지
console.log(flowers[1].classList.contains('changeStyle')); // false 두번째 요소에서 특정 스타일 있는지

// 클래스 토글하기 -> 해당 클래스가있으면 제거, 없으면 추가
...
console.log(parentEl.children[2]); // 개나리 선택
...
console.log(childEl.parentNode.children[2].previousElementSibling); // 장미, :: marker <-li decoration
// 아래 노드 접근
...
console.log(pEl); // 콘솔에서만 존재하는 유령같은 친구 pEl
...
pEl2.innerHTML = '새로 만들어진 두 번째p태그';
container.append(pEl2);

// creatElement를 사용해 만든 요소는 여러번 삽입해도 한 번만 들어감
container.append(pEl, pEl2, pEl2, pEl2); // creatElement로 여러번 만들어야지. 반복해서 넣어도 단 한번 만 들어감.

// div 3개 "안녕"
for (let i = 0; i < 3; i++) {
  // const로 하면 상수, 한바퀴 돌고 바로 에러남.
  // for문 안에서 반복되는 동일한 이름의 변수 생성 가능
  // 왜냐하면 해당 스코프를 돌 때마다 새로운 newDiv 변수가 생성되는 것이고
  // 해당 newDiv 변수는 스코프를 돌면 사라짐
  const newDiv = document.createElement('div'); // 지역 변수 - 스코프 안에서만 살아 있다.
  newDiv.innerHTML = '안녕';
