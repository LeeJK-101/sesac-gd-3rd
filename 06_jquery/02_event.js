// click 이벤트
$('.container').click(() => {
  console.log('event', e); // event 객체
  console.log('event.target', e.target); // event가 발생한 요소(자기 자신)
...
// keyup 이벤트 : 선택한 요소에서 키보드를 때는 이벤트 발생할 경우
// 키보드를 누르고 있는 동안에는 발생하지 않음
...
$('input[name=pw]').keydown((e) => {
  ...
  if (e.key === 'Enter') {
    ...
    console.log(e.target.value); //로그인 로직, 검색 로직 등에서 쓰일 수 있음
  }
});
