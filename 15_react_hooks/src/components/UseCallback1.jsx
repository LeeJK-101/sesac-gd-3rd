...
export default function UseCallback1() {
  ...
  // 그렇기 때문에 isTrue에 변경사항이 있어도 func1이라는 함수는 다시 선언됨
  // useEffect에서 의존성 배열로 func1에 변경사항이 생기면 내부 로직을 실행하도록 작성시
  // 매번 useEffect의 콜백 함수가 실행됨
  const func1 = () => {
    console.log(`number state : ${number}`);
  };

  // 의조넝 배열에 전달한 number의 값이 바꾸지 않을경우
...
// postId값이 바뀌지 않는다면 fetchData 함수는 컴포넌트가 리렌더링 되어도
// 다시 선언되지 않음 -> 계속 사용되지 않는 함수나 연산이 너무 복잡해서
// 계산하는데 만은 자원이 드는 함수는 useCallback으로 저장하면 됨
